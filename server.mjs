...
function publicUser(u, includeInvite=false) { const out={id:u.id,username:u.username,displayName:u.displayName||u.username,partnerId:u.partnerId||null,createdAt:u.createdAt}; if(includeInvite) out.inviteCode=u.inviteCode||null; return out; }
...
function sanitizeSnapshot(data) {
  ...
  if (walks.length > 25000 || landmarks.length > 12000 || visits.length > 120000) throw Object.assign(new Error('Dataset too large'),{status:400});
  return {version:4,walks,landmarks,visits};
}
...
async function handleApi(req,res,url) {
  ...
  try {
    if(url.pathname==='/api/auth/signup' && req.method==='POST') {
      ...
      const username=normalizeUsername(body.username), password=String(body.password||''), displayName=String(body.displayName||'').trim().slice(0,40);
      ...
      await queuedWrite(async()=>{
        ...
        const user={id,username,displayName:displayName||username,salt,passwordHash:await passwordHash(password,salt),inviteCode:freshInviteCode(),partnerId:null,createdAt:Date.now(),snapshot:{version:4,walks:[],landmarks:[],visits:[]},snapshotUpdatedAt:0};
    ...
    if(url.pathname==='/api/snapshot' && req.method==='GET') {
      return send(res,200,{data:user.snapshot||{version:4,walks:[],landmarks:[],visits:[]},updatedAt:user.snapshotUpdatedAt||0},headers);
    }
    ...
    if(url.pathname==='/api/partner-snapshot' && req.method==='GET') {
      ...
      return send(res,200,{partner:publicUser(p),data:p.snapshot||{version:4,walks:[],landmarks:[],visits:[]},updatedAt:p.snapshotUpdatedAt||0},headers);
    }
...
server.listen(PORT,HOST,()=>console.log(`Walk Memory Final running on http://${HOST}:${PORT}`));
