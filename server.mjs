  ...
  try {
    if(url.pathname==='/api/auth/signup' && req.method==='POST') {
      ...
      await queuedWrite(async()=>{
        ...
        const user={id,username,displayName:displayName||username,walkColor,salt,passwordHash:await passwordHash(password,salt),inviteCode:freshInviteCode(),partnerId:null,createdAt:Date.now(),snapshot:{version:4,walks:[],landmarks:[],visits:[]},snapshotUpdatedAt:0};
