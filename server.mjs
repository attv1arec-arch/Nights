...
function sanitizeSnapshot(data) {
  ...
  return {version:5,walks,landmarks,visits,photos};
}
  ...
  try {
    if(url.pathname==='/api/auth/signup' && req.method==='POST') {
      ...
      await queuedWrite(async()=>{
        ...
        const user={id,username,displayName:displayName||username,walkColor,salt,passwordHash:await passwordHash(password,salt),inviteCode:freshInviteCode(),partnerId:null,createdAt:Date.now(),snapshot:{version:5,walks:[],landmarks:[],visits:[],photos:[]},snapshotUpdatedAt:0};
    ...
    if(url.pathname==='/api/snapshot' && req.method==='GET') {
      return send(res,200,{data:user.snapshot||{version:5,walks:[],landmarks:[],visits:[],photos:[]},updatedAt:user.snapshotUpdatedAt||0},headers);
    }
    ...
    if(url.pathname==='/api/partner-snapshot' && req.method==='GET') {
      ...
      return send(res,200,{partner:publicUser(p),data:p.snapshot||{version:5,walks:[],landmarks:[],visits:[],photos:[]},updatedAt:p.snapshotUpdatedAt||0},headers);
    }
