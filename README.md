...
- Removed the fixed red/silver map legend.
- Each account chooses a personal walk color during signup and can change it later.
- Map landmark popups now show estimated walk time/distance from the current location.
- Landmarks support subcategories.
- Landmark list can filter by category, subcategory, and maximum distance; when GPS is available, results sort nearest-first.
