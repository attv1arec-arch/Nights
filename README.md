...
## v6.1 changes
- ETA learning now combines both linked partners’ recorded walking history on both devices, so each person benefits from the other person’s real pace/time-of-day/elevation data.
- Reworked photo capture to reduce Android WebView camera crashes. Native Android uses Capacitor Camera and resizes/compresses photos before local storage or partner upload.
- Separate **Take Photo** and **Gallery** controls for activity photos and landmark photos. The native camera UI provides the phone camera’s normal focus/flash/zoom controls when supported by the device.
- Shared-activity partner photos use the same safer compression/upload path.

## Crash-recovery build

- Active walks and drives are checkpointed to IndexedDB when tracking starts and after every accepted GPS point.
- If the phone dies or the app crashes, the checkpoint is automatically saved as a recovered activity on the next launch.
- GPS points, distance, elevation data, activity photos, and available shared-partner photos are retained.
